...
module.exports = {
    ...
    run : async(client,message,args,prefix) => {
        ...
        if(me)
        {
            ...
            if(flags === '') flags = `${client.emoji.cross} No Badges`;
        ...
        if(!me)
        {
            ...
            let em = new MessageEmbed().setColor(me.displayHexColor).setAuthor({name : `${user.username}'s Information` , iconURL : user.displayAvatarURL({dynamic : true})}).addFields([
                {
                    name : `__General Information__`,
            ...
            return message.channel.send({embeds : [new MessageEmbed().setColor(`#2f3136`).setDescription(`${client.emoji.cross} User not found.`)]})
        }
    }
}
